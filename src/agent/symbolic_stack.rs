...
pub enum SymbolicValue {
    Constant(U256),
    Sload(U256),           // storage slot (for tagging)
    Arithmetic {
        op: ArithmeticOp,
...
impl SymbolicValue {
    /// Recursively check if this value contains a division operation.
    pub fn contains_div(&self) -> bool {
        match self {
            SymbolicValue::Arithmetic { op: ArithmeticOp::Div, .. } => true,
            SymbolicValue::Arithmetic { left, right, .. } => {
                left.contains_div() || right.contains_div()
            }
            _ => false,
        }
    }

    /// Check if this value depends on calldata (attacker-controlled).
    pub fn has_calldata(&self) -> bool {
        match self {
            SymbolicValue::Calldata { .. } => true,
            SymbolicValue::Arithmetic { left, right, .. } => {
                left.has_calldata() || right.has_calldata()
            }
            _ => false,
        }
    }

    /// Check if this value depends on storage (SLOAD).
    pub fn has_sload(&self) -> bool {
        match self {
            SymbolicValue::Sload(_) => true,
            SymbolicValue::Arithmetic { left, right, .. } => {
                left.has_sload() || right.has_sload()
            }
            _ => false,
        }
    }

    /// Extract the constant divisor if this is a division by a constant.
    pub fn extract_divisor(&self) -> Option<U256> {
        if let SymbolicValue::Arithmetic { op: ArithmeticOp::Div, right, .. } = self {
            if let SymbolicValue::Constant(c) = **right {
                return Some(c);
            }
        }
        None
    ...
    /// Extract the constant multiplier from (calldata * constant) or (constant * calldata).
    /// Returns None if the multiplier is not a recognizable constant/sload.
    pub fn extract_multiplier(&self) -> Option<U256> {
        if let SymbolicValue::Arithmetic { op: ArithmeticOp::Mul, left, right } = self {
            // left is calldata, right is constant/sload
            ...
            // right is calldata, left is constant/sload
...
impl SymbolicStack {
    ...
    pub fn swap(&mut self, n: usize) {
        ...
        if len > n {
            ...
            self.stack.swap(top, swap_idx);
        }
    }
    pub fn len(&self) -> usize { self.stack.len() }
    pub fn clone_stack(&self) -> Vec<SymbolicValue> { self.stack.clone() }
}